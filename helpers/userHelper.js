  ...
  const validateAction = (urlid, userId) => {
    ...
    if (typeof urlDatabase[urlid] === "undefined") {
      return ([err, code, msg] = [
        false,
        403,
        `Sorry! ${urlid} short URL is an invalid URL.`,
      ]);
    } else if (urlDatabase[urlid].userID !== userId) {
      return ([err, code, msg] = [
        false,
        403,
        `Sorry! ${urlid} short URL does not belong to you.`,
      ]);
    }
    return ([] = [true]);
  };
  ...
  const validateCredentialsFields = (users, email, password, functionality) => {
    if (email === "" || password === "") {
      return ([err, code, msg] = [
        false,
        400,
        "E-Mail and Password cannot be blank!",
      ]);
    }
    ...
    if (functionality === "login") {
      if (!bcrypt.compareSync(password, user.password)) {
        return ([err, code, msg] = [
          false,
          403,
          `Password does not match ${email}'s saved password`,
        ]);
      }
    }
